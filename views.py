...
from .models import Post, Comment ,Question , Call , Upload ,call_Comment , question_Comment , Search
...
def question_comment_edit(request, question_pk, pk):
	...
	if request.method =='POST':
		...
		if form.is_valid():
			comment = form.save()
			return redirect(question_comment.question)
	else:
...
def call_comment_edit(request, call_pk, pk):
	...
	if call_comment.author != request.user:
		return redirect(call_comment.call)
